@S3 =
	...
	uploadMultiPart: (ops = {}, callback, progressCb) ->
		# include S3.upload description
		# ops.chunkSize [DEFAULT: 5000000] size of chunks to upload (except the last one that is whatever remains) 5MB is the minimum
		# ops.maxParallelUploads [DEFAULT: 5] number of chunks that can be uploaded at once
		...
		ops.chunkSize = Math.max(ops.chunkSize, 5000000)
		...
		_.each ops.files, (file) ->
			...
			if file.size < ops.chunkSize
				return S3.upload(ops, callback, progressCb)
				...
				(error, result) ->
					...
					queueCallback = (e, r) ->
						...
						if (bytePos > file.size)
							Meteor.call "_s3_finish_multipartupload",
								UploadId: UploadId
								callback
							return
						uploadOne()
					...
					uploadOne = () ->
						byteEnd = Math.min(file.size, bytePos + ops.chunkSize)
						S3._uploadOneFile(file_name, ops, file, bytePos, byteEnd, queueCallback, progressCb, partNumber)
						bytePos = byteEnd
						partNumber += 1
	...
	upload: (ops = {}, callback, progressCb) ->
		...
		_.each ops.files, (file) ->
			...
			S3._uploadOneFile(file_name, ops, file, 0, file.size, callback, progressCb)


	_uploadOneFile: (file_name, ops, file, byteStart, byteEnd, callback, progressCb, partNumber) ->
		initial_file_data =
			file:
				name:file_name
				type:file.type
		...
		Meteor.call "_s3_sign",
			...
			file_name: initial_file_data.file.name
			file_type: file.type
			file_size: byteEnd - byteStart
			acl: ops.acl
			bucket: ops.bucket
			expiration: ops.expiration
			UploadId: ops.UploadId
			...
			(error,result) ->
				if result
					...
					S3.collection.update id,
						$set:
							status:"uploading"
					console.log('byteStart, byteEnd', byteStart, byteEnd)
					# Prepare data
					...
					form_data.append "file",file.slice(byteStart, byteEnd)
					...
					xhr.open partNumber ? "PUT" : "POST",result.post_url,true
